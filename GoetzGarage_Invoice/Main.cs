...
using MySql.Data.MySqlClient;//to be able to grab data from teh mysql databas
...
namespace GoetzGarage_Invoice
{
    ...
    public partial class Main : Form
    {
        ...
        public Main()
        {
            ...
            fDataGridHist(); //displays the data from history
        }
        ...
        private void cmbService_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbService.Text == "Oil Change")//we sepeated them because different options willl be avaible if oil was picked
            {
                ...
                try
                {
                    ...
                    command.Connection = connection;
                    int custID = 0;
                    command.CommandText = "Select Max(CustomerID) From tblHistory WHERE Brand='" + cmbBrand.Text + "'AND Model='" + cmbModel.Text + "'AND Year=" + txtYear.Text + "";// this retrevies the most recent history from the database so that it could recommend the oil type of what a person with he same model and make choose
                    custID = (int)command.ExecuteScalar();//there^ we got he inoicenum of that person 
                    command.CommandText = "Select OilType,Warm,Cool From tblHistory WHERE CustomerID=" + custID + "";//and her we used the refernce(invoicenum) to retrive their info so we can display it
                    MySqlDataReader reader = command.ExecuteReader();
            ...
            else if (cmbService.Text == "Brake Work")
            {
                ...
                try
                {
                    ...
                    command.CommandText = "Select Max(CustomerID) From tblHistory WHERE Brand='" + cmbBrand.Text + "'AND Model='" + cmbModel.Text + "'AND Year=" + txtYear.Text + "";// this retrevies the most recent history from the database so that it could recommend the oil type of what a person with he same model and make choose
                    custID = (int)command.ExecuteScalar();//there^ we got he inoicenum of that person 
                    command.CommandText = "Select BrakeType From tblHistory WHERE CustomerID=" + custID + "";
                    MySqlDataReader reader = command.ExecuteReader();
                    while (reader.Read())
                    {
        ...
        public void calc()
        {
            ...
            subtotal += price;
            txtSubtotal.Text = Convert.ToString(subtotal.ToString("0.00"));
            double tax = price * .13;
            txtTax.Text = Convert.ToString(tax.ToString("0.00"));//gets the tax part
            double total = price * 1.13;
            txtTotal.Text = Convert.ToString(total.ToString("0.00"));//gests the final total
        ...
        public void fDataGridHist() //this method displays the history of the suers in the datagrid
        {
            connection.Open();
            MySqlCommand command = new MySqlCommand();
            command.Connection = connection;
            command.CommandText = "Select CustomerID, CustomerName, VIN, Brand, Model, Service From tblhistory ";
            MySqlDataAdapter data = new MySqlDataAdapter(command);
            DataTable dataTable = new DataTable(); 
            data.Fill(dataTable); //fills the datagrid with the query
